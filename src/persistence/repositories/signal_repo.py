...
"""
    ...
    def write_auto_executions(self, rows: Iterable[dict], page_size: int = 200) -> None:
        ...
        for entry in rows:
            ...
            try:
                # §0w P3：走统一 parse_iso_to_utc helper（naive→assume UTC，
                # aware→astimezone 保持绝对时刻），不再用易错的
                # fromisoformat().replace(tzinfo=UTC) 改时区标签反模式。
                from src.utils.timezone import parse_iso_to_utc

                executed_at = parse_iso_to_utc(str(entry.get("at") or ""))
