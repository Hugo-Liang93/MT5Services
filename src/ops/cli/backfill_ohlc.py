...
用法：
    # 回填近 3 个月全部 TF（回测用）
    python -m src.ops.cli.backfill_ohlc --start 2025-12-30 --end 2026-03-30

    # 回填近 6 个月 M5 和 H1
    python -m src.ops.cli.backfill_ohlc --start 2025-09-01 --end 2026-03-30 --tf M5,H1

    # 回填指定品种
    python -m src.ops.cli.backfill_ohlc --symbol XAUUSD --start 2025-01-01 --end 2026-04-05

    # 增量回填（从 DB 已有数据的最后一根 bar 继续）
    python -m src.ops.cli.backfill_ohlc --incremental

    # 仅检查不写入
    python -m src.ops.cli.backfill_ohlc --start 2025-12-30 --end 2026-03-30 --dry-run

    # 查看 DB 当前数据覆盖情况
    python -m src.ops.cli.backfill_ohlc --status
