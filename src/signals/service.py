...
from .strategies import RsiReversionStrategy, SignalStrategy, SmaTrendStrategy
...
class SignalModule:
    ...
    def _persist_signal(
        ...
        if self.repository is None:
            return
        try:
            record = SignalRecord.from_decision(decision, indicators_snapshot=indicators, metadata=metadata)
    ...
    def dispatch_operation(self, operation: str, payload: Optional[Dict[str, Any]] = None) -> Any:
        ...
        handlers = {
            ...
            "summary": lambda: self.summary(hours=payload.get("hours", 24)),
        }
