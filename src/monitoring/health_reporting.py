...
def generate_report_legacy(monitor, hours: int = 24) -> Dict[str, Any]:
    ...
    with monitor._lock:
        ...
        for row in cursor.fetchall():
            component, metric_name, avg_value, min_value = row[:4]
            max_value = avg_value
            sample_count = 1
            last_updated = min_value

            report["components"].setdefault(component, {})
            alert_level = monitor._check_alert_level(component, metric_name, avg_value)
            status = "healthy"
