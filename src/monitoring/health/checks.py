...
def check_data_latency(
    ...
    try:
        ...
        if not latest_bar:
            ...
            latency = max(0.0, (monitor._utc_now() - next_expected_close).total_seconds())
        ...
        logger.error("Failed to check data latency for %s/%s: %s", symbol, timeframe, exc)
        return float("inf")
...
def check_indicator_freshness(
    ...
    try:
        ...
        if not snapshot:
            ...
            freshness = (monitor._utc_now() - monitor._as_utc(snapshot_time)).total_seconds()
    ...
    try:
        ...
        if isinstance(provider_status, dict):
            provider_failures = float(
                sum(
                    int((provider_status.get(name) or {}).get("consecutive_failures", 0))
