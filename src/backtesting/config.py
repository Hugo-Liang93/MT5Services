...
def get_backtest_defaults() -> Dict[str, Any]:
    ...
    if parser.has_section("backtest"):
        _set_float(
            result, parser, "backtest", "default_initial_balance", "initial_balance"
        )
        ...
        _set_int(
            result, parser, "backtest", "max_positions_per_symbol", "max_positions"
        )
        _set_float(
            result, parser, "backtest", "commission_per_lot", "commission_per_lot"
        )
        ...
        _set_bool(
            result, parser, "backtest", "enable_state_machine", "enable_state_machine"
        )
        _set_int(
            result,
            parser,
    ...
    if parser.has_section("filters"):
        ...
        _set_str(
            result, parser, "filters", "allowed_sessions", "filter_allowed_sessions"
        )
        _set_bool(
            result,
            parser,
        ...
        _set_int(
            ...
            parser,
            "filters",
            "session_transition_cooldown_minutes",
        ...
        _set_bool(
            result, parser, "filters", "spread_filter_enabled", "filter_spread_enabled"
        )
        _set_float(
            result, parser, "filters", "max_spread_points", "filter_max_spread_points"
        )
    ...
    if parser.has_section("pending_entry"):
        ...
        _set_int(
            result, parser, "pending_entry", "expiry_bars", "pending_entry_expiry_bars"
        )
        _set_float(
            result,
            parser,
    ...
    if parser.has_section("confidence"):
        ...
        _set_bool(
            result, parser, "confidence", "enable_calibrator", "enable_calibrator"
        )
        _set_bool(
            result, parser, "confidence", "enable_htf_alignment", "enable_htf_alignment"
        )
        _set_float(
            result, parser, "confidence", "htf_alignment_boost", "htf_alignment_boost"
        )
        _set_float(
            result, parser, "confidence", "htf_conflict_penalty", "htf_conflict_penalty"
        )
    ...
    if parser.has_section("circuit_breaker"):
        _set_bool(
            result, parser, "circuit_breaker", "enabled", "circuit_breaker_enabled"
        )
        _set_int(
            result,
            parser,
    ...
    if parser.has_section("monte_carlo"):
        ...
        _set_int(
            result, parser, "monte_carlo", "num_simulations", "monte_carlo_simulations"
        )
        _set_float(
            result,
            parser,
    ...
    try:
        ...
        logger.debug(
            "Failed to load signal config defaults for backtest", exc_info=True
        )
    ...
    try:
        ...
        if risk_config.max_trades_per_hour is not None:
            result.setdefault(
                "max_trades_per_hour", int(risk_config.max_trades_per_hour)
            )
    except Exception:
