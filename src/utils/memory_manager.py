...
class IntelligentMemoryManager:
    ...
    def _estimate_cache_size(self, name: str, cache_obj: Any):
        ...
        try:
            ...
            if hasattr(cache_obj, "__len__"):
                ...
                logger.debug(
                    f"Estimated size for cache '{name}': {estimated_bytes} bytes"
                )
    ...
    def _check_memory_usage(self):
        ...
        try:
            ...
            if usage_percent > 0.9:  # 超过90%
                logger.warning(
                    f"High memory usage: {current_usage / (1024*1024):.1f}MB ({usage_percent:.1%})"
                )
                ...
                logger.info(
                    f"Moderate memory usage: {current_usage / (1024*1024):.1f}MB ({usage_percent:.1%})"
                )
    ...
    def _get_all_cache_stats(self) -> Dict[str, Any]:
        ...
        with self._lock:
            ...
            return {"caches": cache_stats, "summary": summary}
    ...
    def cleanup_old_data(self, max_age_hours: int = 24):
        ...
        with self._lock:
            # 清理旧的历史记录
            self.memory_history = [
                s for s in self.memory_history if s["timestamp"] > cutoff
            ]

            logger.info(
                f"Cleaned memory history, kept {len(self.memory_history)} records"
            )
