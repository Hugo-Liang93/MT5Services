...
async def get_intrabar_indicators(
    ...
    try:
        ...
        logger.error(
            "Failed to get intrabar indicators for %s/%s: %s", symbol, timeframe, exc
        )
        return ApiResponse.error_response(
...
@router.get(
    "/{symbol}/{timeframe}", response_model=ApiResponse[Dict[str, Dict[str, Any]]]
)
...
@router.get(
    "/{symbol}/{timeframe}/{indicator_name}", response_model=ApiResponse[Dict[str, Any]]
)
async def get_indicator(
    ...
    try:
        ...
        logger.error(
            "Failed to get indicator %s for %s/%s: %s",
            indicator_name,
            symbol,
            timeframe,
            exc,
    ...
    try:
        ...
        return ApiResponse.success_response(
            ...
            metadata={
                ...
                "indicators_computed": len(sanitized),
