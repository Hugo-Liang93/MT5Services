...
@router.post(
    "/calendar/refresh", response_model=ApiResponse[EconomicCalendarRefreshModel]
)
def refresh_calendar(
    start_date: Optional[date] = Query(default=None),
    end_date: Optional[date] = Query(default=None),
    job_type: str = Query(
        default="calendar_sync",
        pattern="^(calendar_sync|near_term_sync|release_watch)$",
    ),
...
@router.get(
    "/calendar/upcoming", response_model=ApiResponse[List[EconomicCalendarEventModel]]
)
def upcoming_calendar(
    ...
    return ApiResponse.success_response(
        data=[to_model(item) for item in items],
        metadata={"count": len(items), "hours": hours},
    )


@router.get(
    "/calendar/high-impact",
    response_model=ApiResponse[List[EconomicCalendarEventModel]],
)
...
@router.get(
    "/calendar/curated", response_model=ApiResponse[List[EconomicCalendarEventModel]]
)
...
@router.get(
    "/calendar/risk-windows",
    response_model=ApiResponse[List[EconomicCalendarRiskWindowModel]],
)
def calendar_risk_windows(
    ...
    return ApiResponse.success_response(
        data=[EconomicCalendarRiskWindowModel(**item) for item in windows],
        metadata={"count": len(windows), "hours": hours},
    )


@router.get(
    "/calendar/risk-windows/merged",
    response_model=ApiResponse[List[EconomicCalendarMergedRiskWindowModel]],
)
def merged_calendar_risk_windows(
    ...
    return ApiResponse.success_response(
        data=[EconomicCalendarMergedRiskWindowModel(**item) for item in windows],
        metadata={"count": len(windows), "hours": hours},
    )
...
@router.get(
    "/calendar/trade-guard", response_model=ApiResponse[EconomicCalendarTradeGuardModel]
)
def calendar_trade_guard(
    ...
    result = service.get_trade_guard(
        symbol=symbol,
        at_time=normalize_datetime(at_time),
    ...
    return ApiResponse.success_response(
        ...
        metadata={"data_source": "economic_calendar_service", "symbol": symbol},
    )


@router.get(
    "/calendar/updates", response_model=ApiResponse[List[EconomicCalendarUpdateModel]]
)
def calendar_updates(
    ...
    return ApiResponse.success_response(
        data=[EconomicCalendarUpdateModel(**item) for item in items],
        metadata={"count": len(items), "event_uid": event_uid},
    )
