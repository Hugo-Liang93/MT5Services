...
@router.get(
    "/diagnostics/strategy-conflicts",
    response_model=ApiResponse[StrategyDiagnosticsView],
)
...
@router.get(
    "/diagnostics/daily-report", response_model=ApiResponse[StrategyDiagnosticsView]
)
...
def signal_strategy_audit(
    symbol: Optional[str] = Query(default=None),
    timeframe: Optional[str] = Query(default=None),
    strategy: Optional[str] = Query(default=None, description="单策略过滤"),
    scope: str = Query(default="confirmed", pattern="^(confirmed|preview|all)$"),
    limit: int = Query(default=5000, ge=100, le=10000),
    hours: int = Query(default=168, ge=1, le=24 * 90, description="winrate 统计窗口"),
    conflict_warn_threshold: float = Query(default=0.35, ge=0.0, le=1.0),
    hold_warn_threshold: float = Query(default=0.75, ge=0.0, le=1.0),
    confidence_warn_threshold: float = Query(default=0.45, ge=0.0, le=1.0),
    service: SignalModule = Depends(get_signal_service),
) -> ApiResponse[Dict[str, object]]:
    payload = service.strategy_audit(
        symbol=symbol,
        timeframe=timeframe,
        strategy=strategy,
        scope=scope,
    ...
    return ApiResponse.success_response(
        data=payload,
        metadata={
            "operation": "signal_strategy_audit",
            "symbol": symbol,
            "timeframe": timeframe,
            "strategy": strategy,
            "scope": scope,
...
@router.get(
    ...
    response_model=ApiResponse[SignalMonitoringQualityView],
)
...
def signal_outcomes_winrate(
    ...
    symbol: Optional[str] = Query(default=None),
    quality_tracker: SignalQualityTracker | None = Depends(
        deps.get_optional_signal_quality_tracker
    ),
    trade_tracker: TradeOutcomeTracker | None = Depends(
        deps.get_optional_trade_outcome_tracker
    ),
    service: SignalModule = Depends(get_signal_service),
    ...
    try:
        ...
        if db is None:
            return ApiResponse.error_response(
                "DB not available", error_code="SERVICE_UNAVAILABLE"
            )
