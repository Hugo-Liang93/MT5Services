...
class AggregatedMetrics:
    ...
    def update(self, metrics: IndicatorMetrics) -> None:
        ...
        if metrics.success:
            ...
            self.avg_data_points = (
                self.avg_data_points * (self.successful_computations - 1)
                + metrics.data_points
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        avg_compute_time = self.total_compute_time / successful if successful > 0 else 0
        ...
        success_rate = self.successful_computations / total * 100 if total > 0 else 0
...
class MetricsCollector:
    ...
    def __init__(self, max_history: int = 10000, time_window_hours: int = 24):
        ...
        logger.info(
            f"MetricsCollector initialized: max_history={max_history}, time_window={time_window_hours}h"
        )
    ...
    def record_computation(self, metrics: IndicatorMetrics) -> None:
        ...
        with self.lock:
            ...
            if metrics.name not in self.aggregated_metrics:
                self.aggregated_metrics[metrics.name] = AggregatedMetrics(
                    name=metrics.name
                )
        ...
        with self.lock:
            ...
            if self.metrics_history:
                ...
                if total_successful > 0:
                    total_time = sum(m.compute_time for m in successful_metrics)
                    report["system"]["avg_compute_time_ms"] = (
                        total_time / total_successful * 1000
                    )
                else:
                ...
                incremental_count = sum(
                    1 for m in self.metrics_history if m.incremental
                )
                report["system"][
                    "incremental_rate"
...
def get_global_collector(
    max_history: int = 10000, time_window_hours: int = 24
) -> MetricsCollector:
    """
    获取全局指标收集器实例（单例模式）
    ...
    if _global_collector is None:
        _global_collector = MetricsCollector(
            max_history=max_history, time_window_hours=time_window_hours
        )
