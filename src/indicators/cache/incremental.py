...
class SimpleOHLC:
    ...
    def __init__(
        self, time, open, high, low, close, tick_volume=0, spread=0, real_volume=0
    ):
        self.time = time
        self.open = open
        self.high = high
...
class IncrementalIndicator(ABC):
    ...
    def get_state_key(
        self, symbol: str, timeframe: str, scope: str = "confirmed"
    ) -> str:
        """
        生成状态键
    ...
    def compute(
        ...
        try:
            ...
            should_use_incremental = (
                ...
                and current_state.is_valid(time.time())
                and self._can_use_incremental(bars, current_state)
            )
            ...
            if should_use_incremental:
                ...
                logger.debug(
                    f"Incremental computation: {self.name} for {symbol}/{timeframe}"
                )
            else:
                # 完整计算
            ...
            logger.error(
                f"Indicator computation failed: {self.name}, error: {error_msg}"
            )
    ...
    def _compute_incremental(
        self, bars: List[OHLC], state: IndicatorState
    ) -> Dict[str, float]:
        """
        增量计算（子类可以重写）
    ...
    def _create_new_state(
        self, bars: List[OHLC], result: Dict[str, float]
    ) -> IndicatorState:
        """
        创建新的状态
    ...
    def get_all_states(self) -> Dict[str, Dict[str, Any]]:
        ...
        Returns:
            状态字典
        """
        return {key: state.to_dict() for key, state in self.state_store.items()}
...
class SimpleIndicator(IncrementalIndicator):
    ...
    def _compute_incremental(
        ...
        raise NotImplementedError(
            f"{self.name} does not support incremental computation"
        )
