...
class DependencyManager:
    ...
    def get_all_dependencies(
        self, indicator: str, include_self: bool = False
    ) -> Set[str]:
        """
        获取所有依赖（递归）
    ...
    def get_execution_order(
        ...
        logger.debug(
            f"Execution order computed: {len(result)} levels, {len(relevant_nodes)} nodes"
        )
        return result
        ...
        for node in all_nodes:
            ...
            for dep in self.graph.get(node, set()):
                if dep not in defined_nodes:
                    errors.append(
                        f"Indicator '{node}' depends on undefined indicator '{dep}'"
                    )
    ...
    def export_config(self) -> Dict[str, any]:
        ...
        Returns:
            配置字典
        """
        config = {"indicators": {}, "dependencies": []}
        ...
        for indicator, deps in self.graph.items():
            for dep in deps:
                config["dependencies"].append({"from": dep, "to": indicator})
