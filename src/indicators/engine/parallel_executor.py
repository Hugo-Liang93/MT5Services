...
class ParallelExecutor:
    ...
    def _execute_task(
        self, task_id: str, func: Callable, args: Tuple, kwargs: Dict[str, Any]
    ) -> TaskResult:
        """
        执行单个任务
    ...
    def _execute_task_with_retry(
        ...
        while retry_count <= self.max_retries:
            ...
            logger.warning(
                f"Task {task_id} failed, retrying ({retry_count}/{self.max_retries})"
            )
    ...
    def _task_done_callback(
        self, task_id: str, future: concurrent.futures.Future
    ) -> None:
        """
        任务完成回调
    ...
    def get_stats(self) -> Dict[str, Any]:
        ...
        with self.stats_lock:
            ...
            if stats["completed_tasks"] > 0:
                stats["avg_duration"] = (
                    stats["total_duration"] / stats["completed_tasks"]
                )
            else:
