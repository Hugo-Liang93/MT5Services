...
- `breakout_double_confirm`：`["confirmed"]`（虽然子策略 BollingerBreakout 支持 intrabar，但 JSON 配置为仅 confirmed）
...
- **composites.json 与硬编码不一致**：`breakout_double_confirm` 在 JSON 中有 3 个子策略 + `preferred_scopes=["confirmed"]`，但硬编码回退中只有 2 个子策略 + `("intrabar", "confirmed")`。正常运行时以 JSON 为准
...
- **模块位置注意**：
  ...
  - `HTFTrendM5Entry` → `src/signals/strategies/multi_tf_entry.py`（H1 定方向 + M5 RSI 回调入场联动策略）
