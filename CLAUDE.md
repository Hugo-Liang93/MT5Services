...
**当前投票组配置**：
```
trend_vote     = supertrend, roc_momentum, macd_momentum
reversion_vote = rsi_reversion, cci_reversion, rsi_divergence
breakout_vote  = donchian_breakout, keltner_bb_squeeze, squeeze_release, asian_range_breakout
channel_vote   = bollinger_breakout, fake_breakout, fib_pullback, price_action_reversal
```
...
- **Indicator Exit 已启用**：`indicator_exit_enabled = true`，4 个子规则（SuperTrend 翻转/RSI 极端/MACD 翻转/ADX 塌缩）在持仓期间检测趋势反转收紧 SL
