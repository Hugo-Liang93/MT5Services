...
| 属性 | 轮询模式（默认） | Trigger 模式（子 TF close 驱动） |
|------|-----------------|-------------------------------|
| 触发方式 | BackgroundIngestor 主动轮询 `client.get_ohlc(symbol, tf, 1)` | 子 TF confirmed bar close → 从内存合成父 TF 当前 bar |
| 频率 | `next_intrabar_at`；per-tf > global > `max(5s, tf×5%)` | 子 TF bar 周期（如 H1 由 M5 驱动 = 每 5min） |
| 数据可靠性 | L3 best-effort（MT5 连接可能失败） | **基于 L1 confirmed 事件**（不可丢失） |
| 适用场景 | 未配置 trigger 的 TF | `signal.ini [intrabar_trading.trigger]` 已配置的 TF |

**Trigger 模式配置**（`signal.ini`）：
```ini
...
```
已配置 trigger 的父 TF **不再轮询 MT5**，由 `_synthesize_parent_intrabar()` 从内存缓存合成。

| 共通属性 | 说明 |
|---------|------|
| 数据内容 | N-1 根完整收盘 K 线 + 当前 bar（含最新 OHLC 值）|
| 去重 | Ingestor 层：OHLC 值与上次相同则跳过；指标层：快照与上次完全相同则不发布 |
| 下游管道 | 两种触发方式注入同一管道：`set_intrabar()` → 指标计算 → 信号评估 |
...
intrabar 链路（L3 best-effort）：
  数据源：轮询 MT5 或 子 TF close 合成（trigger 模式）
