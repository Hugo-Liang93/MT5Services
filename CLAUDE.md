...
- **50 个策略**（7 结构化 + 35 legacy + 4 复合 + 4 其他）| **20 个启用指标**（30 个定义）| **16 个 Studio Agent**
...
| `research.ini` | 信号挖掘参数（前瞻周期/过拟合防护/阈值扫描） | 公开 |
...
=== 结构化策略管线（目标精简为 3 层，A9 TODO） ===
raw_confidence（策略内部 Why+When+Where+Volume 已含质量评级 + signal_grade A/B/C）
    × effective_affinity               (保留，值可放宽)
    → max(confidence_floor, result)    (底线保护)
    = final_confidence
    注：htf_alignment 已在策略 Why 层处理，不再外部重复；
        PerformanceTracker/Calibrator 待回测通过后通过装饰器接入
```

### 10 层风控堆栈（从信号到执行，有序）

1. **SignalFilterChain** — 时段/冷却期/点差/经济事件/波动率异常/趋势衰竭过滤
2. **Regime fast-reject** — affinity < 0.10 跳过全部三层评估（结构化策略内置）；affinity < min_affinity_skip 跳过 evaluate()（外部门控）
3. **HTF direction alignment** — Legacy 策略：×0.70/×1.10；结构化策略：Why 层内部处理（TODO A9 移除外部）
4. **ExecutionGate** — voting group 保护、require_armed 门控
5. **PendingEntry** — MT5 挂单（TODO A7：入场职责回归策略层）
...
| 结构化策略（6 策略） | `src/signals/strategies/structured/` |
| Legacy 策略（35+ 类） | `src/signals/strategies/legacy/` |
...
| 挖掘分析器（预测力/阈值/亲和度） | `src/research/analyzers/` |
...
3. `src/signals/strategies/structured/__init__.py` 导出
4. `src/signals/strategies/catalog.py` 注册
5. `signal.ini` + `signal.local.ini` 的 `[strategy_timeframes]` + `[strategy_htf]` **必须同时添加**
6. `tests/signals/` 添加测试
7. 横切关注点（performance/calibrator）通过装饰器接入，**不在策略内部重复实现**

### 新增 Legacy 策略（不推荐，仅维护旧策略）
1. `src/signals/strategies/legacy/` 对应文件实现（必填：`name` / `required_indicators` / `preferred_scopes` / `regime_affinity`）
2. `src/signals/strategies/catalog.py` 注册
3. `src/signals/strategies/__init__.py` 导出
4. `signal.ini` + `signal.local.ini` 的 `[strategy_timeframes]` **必须同时添加**
