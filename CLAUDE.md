...
- **双轨执行体系**：
  - **信号轨**：1 条结构化策略 `StructuredPriceAction`（M15/M30，status=candidate）。2026-04-30 大清场后 13 条结构化策略物理删除，仅保留 PA。
  - **恢复轨**：常驻 `DemoBoundedRecoveryRunner`（tick-derived，独立子系统），按 `risk.ini [recovery_runtime_runner]` 配置驱动；不进入信号管线，直消 `TickFeatureSnapshot`。当前 `demo_only=true`。
  - 详见 `docs/signal-system.md §3.4` 与 `docs/design/full-runtime-dataflow.md §1.1-§1.2`。
- **16 个 Studio Agent**
...
| 需要回答的问题 | 去哪 |
|---|---|
| 当前 sprint 待办 / 本期产出 | `TODO.md` |
| 长期技术方案 / Phase 级规划 | `docs/design/next-plan.md` |
| 已完成归档 / 风险台账 / 历史整改 | `docs/codebase-review.md` |
| 当前运行时事实源 | `docs/architecture.md` · `docs/design/full-runtime-dataflow.md` |
| 时点数据（baseline / gap 分析） | `docs/research/<YYYY-MM-DD>-<topic>.md` |
| 架构决策历史 | `docs/design/adr.md` |
| md 管理规范 | `docs/README.md §3` + 本文 §13 |
...
> 适用于**信号轨**（PA 等 SignalStrategy）。**恢复轨**（martingale / DemoBoundedRecoveryRunner）走独立 admission：`recovery_budgeted` profile + `RecoveryDirectionPolicy` + `RecoveryCostGate` + `RecoveryRiskBudgetGuard` + `RecoveryCalibrationGuard`，不复用本表。详见 `docs/design/full-runtime-dataflow.md §1.2`。
...
3. **HTF direction alignment** — 策略级分层（HtfPolicy 枚举）：HARD_GATE 硬门控 / SOFT_GATE 软门控（冲突时阻止）/ SOFT_BONUS 软加分（冲突时降分不拒绝）/ NONE 不使用。当前 PA = NONE（不依赖 HTF）
4. **ExecutionGate** — intrabar 策略白名单（voting 已废弃，§7 单策略主线）
5. **EntryPolicy** — `(strategy, timeframe)` 路由到 5 个内置 policy：market / pullback / breakout / oco_pullback_breakout / fib_pullback。策略输出 `_exit_spec()`，入场规格由 entry policy 决定（ADR-013）
...
9. **PositionManager + Chandelier Exit** — 统一出场规则（`exit_rules.py`）：初始 SL / Chandelier trailing（current_ATR 动态）/ breakeven + 梯度锁利 / 信号反转 N-bar 确认 / 超时 / 日终平仓。Regime-aware：按 (category × regime) 选择出场 profile。BARRIER 模式（PA 当前）使用固定 SL/TP/timeout，不走 Chandelier
...
**信号轨**（PA 等 SignalStrategy；confirmed-only）：
```
MT5 → BackgroundIngestor → MarketDataService(内存缓存) → StorageWriter(8通道异步) → TimescaleDB
                        ...
                        SignalRuntime(confirmed优先+intrabar)
                          → 结构化策略评估（Why/When/Where 三层 + signal_grade A/B/C）
                                    ↓
                        入场漏斗：
                          SignalFilterChain → ExecutionGate → EntryPolicy(ADR-013)
...
**恢复轨**（martingale / DemoBoundedRecoveryRunner；tick-derived）：
```
MT5 ticks → MarketDataService → TickFeatureEngine → TickFeatureSnapshot → TickFeatureBus
                                    ↓
                        DemoBoundedRecoveryRunner（常驻）
                          → RecoveryDirectionPolicy（auto/buy/sell/skip）
                          → RecoveryCostGate（spread + slippage + commission + min_net_profit）
                          → RecoveryEntrySignalConfirmer（snapshot 连续性）
                          → BoundedRecoveryController（cycle 状态机：open_step / hold / block / close_cycle）
                                    ↓
                        恢复执行：
                          RecoveryPreTradeGuard（policy + cycle + intent 硬上限）
                          → TradeExecutor.execute_recovery_scaling_intent()
                          → RecoveryExecutionAdapter → MT5 真实下单（demo_only=true）
                          → RecoveryCycleStateRecord → recovery_cycle_states（PK 含 account_key + cycle_id）
                          → TradingFlowTraceReadModel（cycle timeline）
```
...
- `builder_phases/` — 构建阶段：market / signal / trading / monitoring / runtime_controls / account_runtime / read_models / **tick_features** / **recovery** / notifications / studio（ADR-010 后 paper_trading 已删除）
- `runtime.py` — AppRuntime：start/stop 生命周期管理
- `mode_controller.py` — 4 种运行模式：FULL/OBSERVE/RISK_OFF/INGEST_ONLY
- `factories/` — 各组件工厂函数（signals / trading / notifications / entry_policies / admission_writeback / market / storage / indicators）
...
| **TimescaleDB** | ~30 表（含 recovery_cycle_states / pending_order_states / position_runtime_states 加 `account_key` 主键） | 全量业务数据 + 三级 retention policy + ADR-012 multi-account 主键契约 |
...
| 结构化策略（**1 类**：`StructuredPriceAction`） | `src/signals/strategies/structured/price_action_m15.py` |
| Mined-rule loader（候选注入通道，未自动启用） | `src/signals/strategies/structured/mined_rule_loader.py` |
| 策略部署契约（4 状态：CANDIDATE/DEMO_VALIDATION/ACTIVE_GUARDED/ACTIVE） | `src/signals/contracts/deployment.py` |
...
| 组件 | 路径 |
|------|------|
| TickFeatureEngine | `src/market/tick_features/engine.py` |
| TickFeatureCalculator | `src/market/tick_features/calculator.py` |
| TickFeatureBus（多消费者订阅） | `src/market/tick_features/bus.py` |
| TickFeatureSnapshot 数据结构 | `src/market/tick_features/models.py` |
| TickFeatureHealthStore | `src/market/tick_features/health.py` |
...
| EntryPolicyRegistry（5 内置 policy，ADR-013） | `src/trading/entry_policy/` |

**恢复子系统（tick-derived bounded recovery / martingale）**

| 组件 | 路径 |
|------|------|
| DemoBoundedRecoveryRunner（常驻） | `src/trading/recovery/runner.py` |
| RecoveryPolicy / RecoveryCycleState | `src/trading/recovery/models.py` |
| 一次性 canary 入口 | `src/trading/recovery/canary.py` + `src/ops/cli/recovery_canary.py` |
| RecoveryDirectionPolicy / CostGate / EntrySignalConfirmer | `src/trading/recovery/entry_policies.py` |
| BoundedRecoveryController（cycle 状态机） | `src/trading/recovery/controller.py` |
| RecoveryExecutionAdapter | `src/trading/recovery/execution.py` |
| RecoveryRiskBudgetGuard | `src/trading/recovery/risk_budget.py` |
| RecoveryCalibrationGuard（实仓样本校准） | `src/trading/recovery/calibration_guard.py` |
| RecoveryExposureLedger | `src/trading/recovery/exposure_ledger.py` |
| RecoveryDecisionAnalytics（status() 分析） | `src/trading/recovery/analytics.py` |
...
- ~~**generate_report(hours=24)**：ring_size=2400 仅覆盖 ~3.3h~~ ✅ 已修：ring_size 17280（24h 5s 精度，metrics_store.py:29 + monitor.py:46）
...
- ADR-013: 入场决策从信号策略中收口为可插拔 EntryPolicy 端口（策略只输出 `_exit_spec()`；入场规格由 `(strategy, timeframe)` 路由到 5 个内置 policy）
