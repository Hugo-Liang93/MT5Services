...
│   │   ├── analytics/        # 诊断分析工具（DiagnosticsEngine）
│   │   ├── contracts/        # 接口/常量定义（sessions.py）
│   │   ├── evaluation/       # Regime 分类、置信度校准、指标辅助
...
7. **SignalModule** — 策略注册与评估引擎
8. **SignalRuntime** — 事件驱动信号运行时（`src/signals/orchestration/`）
9. **TradeExecutor** — 信号自动交易执行器
10. **PositionManager** — 持仓监控与管理
11. **MonitoringManager** — 健康检查与巡检
...
| 属性 | 说明 |
|------|------|
| 触发条件 | 新 tick 到达（由 BackgroundIngestor 推送），但经过节流才真正计算 |
| 节流规则 | `min_gap = max(1s, bar时长 × 2%)`，M1→1.2s，M5→6s，M15→18s，H1→72s |
| 数据内容 | N-1 根完整收盘 K 线 + 当前 bar（含最新 tick 的 open/high/low/close/volume）|
| 去重机制 | 若计算结果与上次快照完全相同（bar_time 相同且所有指标值不变），**不重新发布**，SignalRuntime 继续持有上次状态 |
...
```
MT5 tick
  ↓ BackgroundIngestor（节流：min_gap = max(1s, bar时长×2%)）
  ↓
 ...
 _intrabar_loop 独立线程
  ...
  ↓
 仅 intrabar_eligible=true 的指标（当前 8 个）
  ↓
...
**`indicators.json` 中没有 `intrabar_eligible` 字段。** Intrabar 计算哪些指标完全由策略的 `preferred_scopes` + `required_indicators` 在启动时自动推导：

```
intrabar_eligible(指标) = 是否存在任一策略：
    "intrabar" ∈ preferred_scopes  AND  该指标 ∈ required_indicators
```
...
| `boll20` | bollinger_breakout, keltner_bollinger_squeeze, breakout_double_confirm | 盘中触及/突破通道边界即可预警 |
| `keltner20` | keltner_bollinger_squeeze | BB/KC 挤压是实时状态 |
...
| `keltner_bollinger_squeeze` | boll20, keltner20 | BB/KC 挤压形态 |
| `breakout_double_confirm` | boll20, donchian20, adx14 | 双通道确认突破（复合：BollingerBreakout + DonchianBreakout）|

### 各类指标的语义分析

不同指标在未收盘的 bar 上计算是否有意义，是 intrabar_eligible 的语义依据：
...
当 `UnifiedIndicatorManager` 独立运行（无 `SignalModule`，如纯 API 查询场景）时，`set_intrabar_eligible_override()` 不会被调用。此时 `_get_intrabar_eligible_names()` 回退到默认行为：所有 `enabled=true` 的指标均参与 intrabar 计算。这是 backward-compatible 的设计，保证独立部署不会因缺少策略注册而出现空集。
...
| `keltner_bollinger_squeeze` | boll20, keltner20 | intrabar + confirmed |
...
### Indicator Delta（`src/indicators/manager.py`）
...
raw_confidence（策略规则输出）
    ...
    → ConfidenceCalibrator（分阶段校准，基于历史胜率混合）
    = final_confidence
```
...
| `vwap30` | 成交量 | — | 30 周期 VWAP |

> Demo 账户无真实成交量，`mfi14`/`obv30` 默认 `enabled: false`。
