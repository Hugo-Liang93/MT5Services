...
**`src/ops/` 分类**：
- `cli/` — 24 个 CLI（`backtest_runner` / `diagnose_no_trades` / `mining_runner` / `aggression_search` / `walkforward_runner` / `correlation_runner` / `backfill_ohlc` / `confidence_check` / `health_check` / `live_preflight` / `demo_vs_backtest` / `pipeline_gate_audit` / `sltp_grid_search` / `exit_experiment` / `reset_database` / `nightly_wf` / `mining_walk_forward` / `daily_report` / `aggression_scan` / `pa_barrier_grid` / `test_notification` / `import_historical_mining` + `_coverage` / `_persistence` 辅助）
