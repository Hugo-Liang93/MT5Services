...
│  │    │                                                                  │   │
│  │    ├─ 4f. HTF 指标注入                                                │   │
│  │    │       消费: strategy.htf_indicators 声明 + IndicatorManager     │   │
...
│  │    │                                                                  │   │
│  │    ├─ 4i. HTF 方向对齐修正                                            │   │
│  │    │       消费: htf_direction_fn(symbol, tf) → HTFStateCache         │   │
│  │    │       逻辑: 对齐 → confidence × 1.10                             │   │
│  │    │             冲突 → confidence × 0.70                             │   │
...
│  │  产出: TradeParameters                                                │   │
│  │    ├─ entry_price                                                     │   │
│  │    ├─ stop_loss   (ATR × SL倍数, 按 TF 差异化: M1=1.0, H1=1.5)     │   │
│  │    ├─ take_profit (ATR × TP倍数, 按 TF 差异化: M1=2.0, H1=3.0)     │   │
...
│  │  │     │ BrokerConstraintRule: stops_level/freeze_level    │      │  │   │
│  │  │     │                                                   │      │  │   │
...
```
  strategy.evaluate(context)
    ...
    max(confidence_floor, result)
         ...
         │
    ×  intrabar_confidence_decay (scope=intrabar 时, 默认 0.85)
         ▼
    ×  htf_alignment_multiplier
         │  对齐: × 1.10  |  冲突: × 0.70  |  无数据: × 1.0
         ▼
    min(1.0, result)
         ▼ ═══════════════════════════════════════════════
