...
> **本文件记录已确定的设计决策。修改涉及 ADR 的组件前必须先读本文件，评估影响后再行动。**
> 新增决策时复制模板，编号递增。变更已有决策须注明原因和日期。
...
**如果将来需要异步**：不应改 bus 本身，而是让慢 listener 自行内部缓冲（自带队列+消费线程）。Bus 层保持同步，慢消费者自己负责解耦。
