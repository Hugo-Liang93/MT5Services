...
第一阶段只使用 baseline backtest 的 `raw_results.trades` 训练 Entry Meta，不读取 demo/live runtime 状态，也不把模型接入真实下单链路。该实验只进入 Research + Backtest overlay，用离线回测证明其对既有策略 entry 的过滤价值。
