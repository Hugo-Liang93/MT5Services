...
- `api.py`
  - 仅作为回测 HTTP 组合根
  - 不承载任务状态、模板生成、推荐逻辑
- `api_routes/config.py`
  - 回测配置默认值
  - 参数空间模板查询
- `api_routes/jobs.py`
  - 回测/优化/前推任务提交
  - 任务状态与历史结果查询
- `api_routes/recommendations.py`
  - 参数推荐生成、审核、应用、回滚
- `api_config.py`
  - 回测请求模型
  - 配置解析与模板生成
...
3. `api.py` 只允许组合子路由，不允许回流任务执行、配置模板、推荐审核等实现细节。
4. 路由层只做 HTTP 协议适配；运行态存储、任务执行、推荐逻辑分别落在独立模块。
5. 新增回测能力时，优先归入现有协作者文件；只有形成稳定新职责时才新增文件。
