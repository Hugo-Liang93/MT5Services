...
| 看策略开发接口、regime、单策略主线、TF 参数和领域规则 | 本文 |
...
│   ├── policy.py              # SignalPolicy + RuntimeSignalState
...
稳定计数满足阈值时发布 intrabar_armed 信号
随后统一进入 ExecutionIntentPublisher → ExecutionIntentConsumer → TradeExecutor
```
...
当前系统已移除 runtime/backtest 中的 vote/consensus 层，正式主链路为：

1. `SignalRuntime` 对每个目标策略独立评估；
2. `decision.direction not in ("buy", "sell")` 时直接落 `no_signal`；
3. `buy/sell` 决策直接进入状态机、持久化、trace 与执行链路；
4. research/mining 默认只产出单策略候选，不生成 voting group 或 `strategy="consensus"` 工件。

这意味着：

- `config/signal.ini` 不再支持 `[voting]`、`[voting_groups]`、`[standalone_override]`；
- `config/signal.local.ini` 若仍保留这些旧 section，会在启动时按配置漂移直接失败；
- `ExecutionGate` 只保留 intrabar 交易门禁，不再承担“投票组成员禁止下单”的职责。
...
SignalRuntime._publish_signal_event(event)
    ├─→ ExecutionIntentPublisher.on_signal_event()   (confirmed / intrabar_armed 统一发布 intent)
    ├─→ SignalQualityTracker.on_signal_event()        (信号质量追踪)
    └─→ HTFStateCache.on_signal_event()               (HTF 方向缓存)

ExecutionIntentConsumer._process_intent()
    └─→ TradeExecutor.process_event()                 (worker / 单账户执行入口)
```

**终态事件所有权**：

- `TradeExecutor` 负责返回正式执行结果：`completed / skipped / failed`；
- 执行器内部只发阶段性事实，如 `execution_blocked`、`execution_submitted`、`admission_report`；
- execution intent 主链的 terminal pipeline 事件只由 `ExecutionIntentConsumer` 发一次：
  - `execution_succeeded`
  - `execution_skipped`
  - `execution_failed`
- 单账户/本地 `on_signal_event()`、`on_intrabar_trade_signal()` 适配路径不再自成一套语义，而是复用同一套 terminal 结果解释与发射服务；因此本地运行和 worker 消费在 trace 上看到的是同一合同。

这样 execution intent 的完成状态与 trace 终态保持单一事实源，不再由执行器本地和 consumer 双方重复补发。
...
```
ExecutionIntentPublisher.on_signal_event(event)
  → 仅接受 confirmed_* 与 intrabar_armed_* 两类可执行信号
  → 写入 execution_intents

ExecutionIntentConsumer._process_intent(intent)
  → 反序列化为 SignalEvent
  → TradeExecutor.process_event(event)
      → scope="intrabar" 时进入 _handle_intrabar_entry()
      → IntrabarTradeGuard.can_trade()  — 同 bar/策略/方向只允许一次
      → ExecutionGate.check_intrabar()  — 策略白名单 / session / deployment 门禁
      → _run_pre_trade_filters()        — 复用完整 pre-trade filter chain
