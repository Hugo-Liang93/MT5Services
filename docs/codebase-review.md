...
`BacktestEngine` 不注入 `tick_features` 到 `ctx.indicators`（tick_features 只存在于
runtime 路径，回测里走 OHLC bar 驱动）。`PA._post_confidence_modifier` 在回测中
**永远 modifier=1.0**（noop）。所以 D 方向只能在 live runtime 上验证，
不能用 backtest grid 验证。**D 方向跳过本轮回测扫描**。

### C 方向实测：BARRIER vs CHANDELIER 1 月对比

工具补齐：[pa_barrier_grid.py](../src/ops/cli/pa_barrier_grid.py) 加 `--exit-mode {barrier,chandelier,both}` + `--aggression` 参数。
[price_action_m15.py](../src/signals/strategies/structured/price_action_m15.py) 加 `_exit_mode` 类变量（默认 barrier 不破坏现状）。

测试：tf=M15, 2025-04-01~2025-05-01（1 月），相同入场参数（sl=1.5/tp=2.5/tb=20/adx=12）：

| Mode | α | Trades | WR | PnL | **PF** | Sharpe | **MaxDD** |
|---|---|---|---|---|---|---|---|
| barrier | – | 97 | 28.9% | -535 | 0.39 | -2.60 | 27.07% |
| chandelier | 0.30 | 94 | 38.3% | -477 | 0.39 | -2.47 | 24.29% |
| **chandelier** | **0.50** | 93 | **35.5%** | **-447** | **0.46** | -2.03 | **22.97%** |
| chandelier | 0.70 | 98 | 37.8% | -491 | 0.46 | -2.04 | 25.04% |

**结论**：Chandelier α=0.5 略优于 BARRIER（PF +18% / DD -15% / WR +23%），
但仍**全部 PF<1.0** —— Chandelier 救不了 PA。

### 真正的瓶颈：入场质量

C 方向数据证实 PA 核心问题不在出场，**在 _when 入场过密 + _why 信号粗糙**。
- 1167 trades / 1y = 3.2/day，密度过高
- WR 26.7%（barrier）~ 35.5%（chandelier）—— 都低
- W/L ratio 不足以弥补低 WR

下一步必须做 **A 方向**（收紧 _when 多形态共振）或 **B 方向**（重设计 _score 加 vetting）。
两者都涉及策略代码逻辑改动（不是参数搜索）。
