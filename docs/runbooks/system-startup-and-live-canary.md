...
8. 若本次是 bounded recovery / martingale canary：
   ...
   - dry-run 阶段必须看到 `recovery_cycle_states` 有目标 `account_key + cycle_id` 记录，`trade_command_audits` 出现 `operation=execute_trade` 且 `request_payload.metadata.execution_scope=recovery_scaling`。若使用 `--live-cycle` 且未提交真实初始仓，CLI 必须输出 `dry_run_cycle_finalization.status=closed`，并且 canary 结束后不应留下 `tick_martingale_probe` 的 open recovery cycle。若使用 `--submit-recovery-order`，必须看到 `current_step.status=submitted`、`recovery_cleanup.status=closed`，以及 `close_initial` / `close_step_1` 两个 cleanup action；结束后 MT5 XAUUSD 持仓/挂单和 open recovery cycle 都必须为 0。
