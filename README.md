...
Phase 3 also enables a soft-regime path. When `config/signal.ini -> [regime].soft_regime_enabled = true`, runtime metadata now includes `regime_source`, `regime_probabilities`, and `dominant_regime_probability`, and affinity is computed from the weighted regime distribution instead of one hard class.
