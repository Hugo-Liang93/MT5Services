...
#### Phase 2：定时任务 + 零侵入适配器（下一步）

- [ ] **定时日报调度器**：每天 UTC `[schedules] daily_report_utc` 触发，调用 `/v1/trade/daily_summary` 内部 API，渲染 `info_daily_report` 模板并推送
- [ ] **TradingStateAlerts 轮询适配器**：每 60s 调用 `summary()`，新增的 `pending_missing` / `pending_orphan` / `unmanaged_live_positions` 映射为 NotificationEvent（零侵入）
