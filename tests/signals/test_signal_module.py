...
def test_signal_module_exposes_strategy_requirements() -> None:
    ...
    assert set(requirements) == {"structured_price_action"}
...
def test_removed_micro_momentum_is_not_in_strategy_catalog() -> None:
    module = SignalModule(indicator_source=DummyIndicatorSource())

    with pytest.raises(ValueError, match="unsupported signal strategy"):
        module.strategy_capability("structured_micro_momentum")
...
def test_intrabar_required_indicators_derives_from_strategy_scopes() -> None:
    ...
    当前 PA-only 策略池使用 confirmed bar；intrabar 是增强支线。
    confirmed-only 策略不应把自身指标加入 intrabar runtime 需求。
    """
    module = SignalModule(
        indicator_source=DummyIndicatorSource(),
