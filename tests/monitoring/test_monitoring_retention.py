...
def test_monitoring_manager_runs_retention_for_health_and_events() -> None:
    ...
    health_monitor = SimpleNamespace(
        cleanup_old_data=lambda days: calls.append(("health", days))
    )
    indicator_manager = SimpleNamespace(
        cleanup_old_events=lambda days: calls.append(("events", days))
    )

    manager = MonitoringManager(health_monitor, check_interval=5)
    manager.retention_interval_seconds = 0
    manager._monitored_components["indicator_calculation"] = {
        "obj": indicator_manager,
        "methods": [],
    }
