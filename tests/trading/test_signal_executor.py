...
def test_trade_executor_allows_cross_tf_natural_hedge() -> None:
    ...
    per-strategy reversal history 协同清算 (exit_rules.check_signal_reversal)
    在反向积累后整 stack market close。
    """
    module = DummyTradingModule()
    tracked_positions = [
        {
    ...
    # 不同 TF (M30) 反向 buy 信号 — 现在允许形成 cross-TF hedge
...
def test_trade_executor_pending_submission_sets_reentry_cooldown_anchor() -> None:
    ...
    # 4-tuple key: (symbol, timeframe, strategy, direction)
    assert executor._last_entry_bar_time[
        ("XAUUSD", "M5", "sma_trend", "buy")
