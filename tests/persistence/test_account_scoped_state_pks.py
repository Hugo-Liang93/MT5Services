...
from src.persistence.schema.pending_order_states import DDL as PENDING_ORDER_STATES_DDL
...
def test_pending_order_states_pk_includes_account_scope() -> None:
    ...
    assert (
        "order_ticket bigint primary key" not in ddl
...
def test_pending_order_states_upsert_conflict_target_includes_account_scope() -> None:
    ...
    assert (
        "on conflict (order_ticket)" not in sql
...
def test_position_runtime_states_pk_includes_account_scope() -> None:
    ...
    assert (
        "position_ticket bigint primary key" not in ddl
...
def test_position_runtime_states_upsert_conflict_target_includes_account_scope() -> (
    None
):
    sql = _normalize(UPSERT_POSITION_RUNTIME_STATES_SQL)
    assert (
        "on conflict (position_ticket)" not in sql
...
def test_trade_control_state_pk_matches_unique_index_to_avoid_alias_rename_failure() -> (
    None
):
    ...
    assert (
        "account_alias text primary key" not in ddl
    ), "PK 不能仅是 account_alias（不全局唯一/可重命名 → unique index 冲突写失败）"
    # 新 PK 必须以 account_key 为锚（globally unique env:server:login）
    assert "account_key text not null primary key" in ddl or re.search(
        r"primary key\s*\(\s*account_key\s*\)", ddl
