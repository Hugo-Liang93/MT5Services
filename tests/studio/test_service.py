...
def test_register_and_build_agents() -> None:
    ...
    studio.register_agent(
        "collector", lambda: build_agent("collector", "working", "采集中")
    )
...
def test_failing_provider_returns_error_agent() -> None:
    studio = StudioService()
    studio.register_agent(
        "collector", lambda: (_ for _ in ()).throw(RuntimeError("boom"))
    )
...
def test_build_summary() -> None:
    studio = StudioService()
    studio.register_agent(
        "collector", lambda: build_agent("collector", "working", "ok")
    )
    studio.register_agent(
        "analyst", lambda: build_agent("analyst", "error", "fail", alert_level="error")
    )
...
def test_overall_health_logic() -> None:
    ...
    # Warning
    studio.register_agent(
        "b", lambda: build_agent("b", "warning", "warn", alert_level="warning")
    )
    assert studio.build_summary()["health"] == "degraded"

    # Error overrides warning
    studio.register_agent(
        "c", lambda: build_agent("c", "error", "fail", alert_level="error")
    )
