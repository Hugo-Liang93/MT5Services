...
from src.research.external.yfinance_client import YFinanceClient, YFinanceError
...
def test_history_for_bumps_end_by_one_day_for_inclusive_range() -> None:
    ...
    with patch.dict(
        ...
        {
            "yfinance": MagicMock(
                Ticker=MagicMock(return_value=MagicMock(history=mock_yf_call))
            )
        },
    ):
    ...
    assert (
        yf_kwargs.get("start") == "2026-04-01"
    ), f"Expected start='2026-04-01', got {yf_kwargs.get('start')}"
    assert (
        yf_kwargs.get("end") == "2026-04-03"
    ), f"Expected end='2026-04-03' (bumped from 2026-04-02), got {yf_kwargs.get('end')}"
