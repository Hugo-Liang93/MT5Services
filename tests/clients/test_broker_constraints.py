...
class TestValidateTradeRequestBrokerIntegration:
    ...
    def test_validate_raises_on_trade_mode_disabled(self):
        ...
        with patch("src.clients.mt5_trading.mt5") as mock_mt5:
            ...
            with pytest.raises(
                MT5TradingClientError, match="Broker constraint violated"
            ):
...
class TestPrecheckBrokerConstraints:
    ...
    def test_precheck_blocked_by_trade_mode(self):
        client = DummyTradingClientWithBrokerChecks(
            broker_checks=[
                {
                    "name": "trade_mode",
                    "passed": False,
                    "message": "Trading disabled (DISABLED)",
                },
            ]
        )
        service = TradingService(
            client=client, pre_trade_risk_service=DummyRiskService()
        )
        ...
        assert any(
            c["name"] == "trade_mode" and not c["passed"] for c in result["checks"]
        )
        ...
        client = DummyTradingClientWithBrokerChecks(
            broker_checks=[
                {"name": "trade_mode", "passed": True, "message": "ok"},
                {
                    "name": "stops_level_sl",
                    "passed": False,
                    "message": "SL too close: 0.20 < 0.50",
                },
            ]
        )
        service = TradingService(
    ...
    def test_precheck_passes_all_broker_checks(self):
        ...
        broker_checks = [
            ...
            if c["name"].startswith(("trade_mode", "stops_level"))
        ]
        ...
        client = DummyTradingClientWithBrokerChecks(
            broker_checks=[
                {"name": "trade_mode", "passed": True, "message": "ok"},
                {
                    "name": "freeze_level",
                    "passed": True,
                    "message": "Freeze level = 20 points",
                },
            ]
        )
        service = TradingService(
