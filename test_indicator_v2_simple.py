...
def test_smart_cache():
    ...
    smart_cache_module = importlib.util.module_from_spec(spec)
    sys.modules["smart_cache"] = smart_cache_module
    spec.loader.exec_module(smart_cache_module)
    
    from smart_cache import SmartCache, get_global_cache
...
def test_metrics_collector():
    ...
    from metrics_collector import get_global_collector, record_indicator_computation
...
def test_incremental_base():
    ...
    spec = importlib.util.spec_from_file_location(
        "incremental",
        "src/indicators_v2/base/incremental.py"
    )
    incremental_module = importlib.util.module_from_spec(spec)
    sys.modules["incremental"] = incremental_module
    spec.loader.exec_module(incremental_module)
    
    from incremental import IncrementalIndicator, SimpleIndicator
